...
- [x] Created root marketplace manifest `.claude-plugin/marketplace.json` registering all 18 verified external GitHub skill repositories with categories and UI tags.
...
- [x] Audited all 40 user-starred GitHub repositories from `https://github.com/apirak-k?tab=stars` and mapped them into 3 distinct categories.
- [x] Embedded all 18 authentic upstream GitHub repositories as official Git Submodules in `plugins/haws/skills/` to enable true 1-Click Installation (`/plugin install haws@haws-marketplace`).
...
## Confirmed decisions
- **1-Click Install via Git Submodules**: All 18 external open-source skills are embedded directly as official Git Submodules (`.gitmodules`) from their original upstream GitHub repositories so `/plugin install haws@haws-marketplace` installs rules, 4 subagents, and all 18 authentic skills in one click.
