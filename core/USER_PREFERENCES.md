...
- **Genuine Slash Command Execution (No Performative Tagging)**: Do NOT prepend artificial cosmetic labels (e.g. `[Auto-Skill: ...]`). When applying a skill, directly execute its actual workflow rigorously and clearly state the active Slash Command (e.g. "กำลังใช้งาน `/grill-me`...", "กำลังใช้งาน `/review`...").
...
- **Sub-Second Native Skill Inspection**: When checking installed skills or system health, always use the high-performance native checker (`scripts/check-skills.ps1` / `scripts/check-skills.sh`) to provide instant sub-second reports (< 0.5s) without slow shell loops.
...
- **Token Budget Guardrail & Dangerous Level Alert**: The AI and `@organizer` must monitor Antigravity skill customization token consumption via `scripts/check-skills.ps1` and immediately alert the user in chat whenever usage reaches dangerous thresholds (>= 75% or 15,000 tokens) along with the Top 5 largest skills for compaction.
