...
- **Git Remote Control & No Auto-Push**: AI agents must NEVER run `git push` to GitHub or any remote repository autonomously without first explicitly asking for human confirmation and receiving direct user approval in chat. Local git commits and working tree edits are permitted, but publishing to remote is strictly controlled by the human user.
