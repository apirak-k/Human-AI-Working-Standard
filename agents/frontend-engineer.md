...
- Match UI/UX tasks to relevant design, frontend, accessibility, or documentation capabilities available for the task.
...
- **Reporting Return**: Always return task outcomes strictly wrapped in `<task_report>`:
  ...
  - **Evidence**: Report build, lint, and browser checks actually run; mark unavailable or unrun checks `[Unverified]`.
