...
test_reset_defaults_reenables_detected_environments() {
    mkdir -p "${FIXTURE_HOME}/.claude" "${FIXTURE_PROJECT}/ai-configs"
    printf '%s\n' 'claude' > "${FIXTURE_PROJECT}/ai-configs/environments.disabled"
    source_haws || return 1
    settings_draft_load || return 1
    ! printf '%s\n' "${HAWS_DRAFT_ENVIRONMENTS}" | grep -Fx 'claude' >/dev/null 2>&1 || return 1

    interactive_menu() {
        INTERACTIVE_MENU_SELECTION=0
        return 0
    }
    settings_draft_reset || return 1
    printf '%s\n' "${HAWS_DRAFT_ENVIRONMENTS}" | grep -Fx 'claude' >/dev/null 2>&1 || return 1

    export HAWS_TEST_NO_INTEGRATION=1
    settings_plan_build || return 1
    settings_apply_final >/dev/null 2>&1 || return 1
    unset HAWS_TEST_NO_INTEGRATION
    disabled_environments_load || return 1
    [ -z "${DISABLED_ENVIRONMENTS[claude]:-}" ] || return 1
    ! grep -E '^[[:space:]]*claude[[:space:]]*$' \
        "${FIXTURE_PROJECT}/ai-configs/environments.disabled" >/dev/null 2>&1
}

test_reset_defaults_restores_repository_skill_baseline() {
    mkdir -p "${FIXTURE_HOME}/.claude" "${FIXTURE_PROJECT}/.haws/state" \
        "${FIXTURE_PROJECT}/ai-configs" "${FIXTURE_PROJECT}/skills"
    printf '%s\n' \
        '# HAWS Disabled Skills (source-aware)' \
        'custom::skills/custom::demo-one' > "${FIXTURE_PROJECT}/skills/skills.disabled"
    printf '%s\n' '# HAWS Disabled Skills (source-aware)' \
        > "${FIXTURE_PROJECT}/.haws/state/skills.disabled"
    printf '%s\n' 'claude' > "${FIXTURE_PROJECT}/ai-configs/environments.disabled"
    source_haws || return 1
    settings_draft_load || return 1
    ...
    settings_draft_reset || return 1
    ! printf '%s\n' "${HAWS_DRAFT_SKILLS}" | \
        grep -Fx 'custom::skills/custom::demo-one' >/dev/null 2>&1 || return 1
    printf '%s\n' "${HAWS_DRAFT_ENVIRONMENTS}" | \
        grep -Fx 'claude' >/dev/null 2>&1 || return 1
    ...
    settings_apply_final >/dev/null 2>&1 || {
        unset HAWS_TEST_NO_INTEGRATION
        return 1
    }
    unset HAWS_TEST_NO_INTEGRATION
    grep -Fx 'custom::skills/custom::demo-one' \
        "${FIXTURE_PROJECT}/.haws/state/skills.disabled" >/dev/null 2>&1 || return 1
    grep -Fx 'custom::skills/custom::demo-one' \
        "${FIXTURE_PROJECT}/skills/skills.disabled" >/dev/null 2>&1 || return 1
    ! grep -Fx 'claude' "${FIXTURE_PROJECT}/ai-configs/environments.disabled" \
        >/dev/null 2>&1
}
...
test_local_state_paths_are_ignored() {
    ...
    grep -F 'environments.disabled' "${PROJECT_ROOT}/.gitignore" >/dev/null || return 1
    grep -F '/secondbrain/notes/' "${PROJECT_ROOT}/.gitignore" >/dev/null
}
...
run_test test_reset_defaults_reenables_detected_environments
run_test test_reset_defaults_restores_repository_skill_baseline
