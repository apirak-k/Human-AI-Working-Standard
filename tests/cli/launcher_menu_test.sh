...
test_arrow_space_enter_updates_only_fixture() {
    ...
    printf '%b' "${down}${down}\n\n${down} \n\nq" |
        HOME="${FIXTURE_HOME}" bash "${FIXTURE_PROJECT}/haws.sh" skills >"${OUTPUT_FILE}" 2>&1 || return 1
    assert_file_contains "${FIXTURE_PROJECT}/.haws/state/skills.disabled" 'demo-alpha' || return 1
...
test_checklist_eof_cancels_without_saving() {
    ...
    assert_file_not_exists "${FIXTURE_PROJECT}/.haws/state/skills.disabled"
}
