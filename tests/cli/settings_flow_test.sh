...
seed_local_second_brain_defaults() {
    local brain="${FIXTURE_PROJECT}/secondbrain"
    mkdir -p "${brain}" || return 1
    printf '%s\n' '# Preferences' > "${brain}/USER_PREFERENCES.md"
    printf '%s\n' '# Anti-patterns' > "${brain}/ANTI_PATTERNS.md"
...
test_preview_screen_renders_compact_packs_and_singles_without_paths() {
    ...
    assert_output_contains 'Custom Skills:' || return 1
...
test_interactive_skill_pages_use_settings_header_and_fixed_state_columns() {
    ...
    input+="${down}\n\nq"
    run_haws_input "${input}" || true
    assert_output_contains '                       Configure Custom Skills' || return 1
    assert_output_not_contains '=== Configure Custom Skills ===' || return 1
}
